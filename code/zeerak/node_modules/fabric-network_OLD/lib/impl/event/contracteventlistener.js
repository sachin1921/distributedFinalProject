...
class ContractEventListener extends AbstractEventListener {
    ...
    async register() {
        ...
        this._registration = this.eventHub.registerChaincodeEvent(this.contract.getChaincodeId(), this.eventName, this._onEvent.bind(this), this._onError.bind(this), this.clientOptions);
    ...
    async _onEvents(events) {
        ...
        if (!this.options.asArray) {
            ...
            await Promise.all(events.map((event) => this._onEvent(event.chaincode_event, event.block_num, event.tx_id, event.tx_status, events.length)));
        }
        else {
